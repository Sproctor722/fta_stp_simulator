...
def build_claim_package(recovery_df: pd.DataFrame, duties_df: pd.DataFrame,
    ...
    for _, row in recovery_df.iterrows():
        ...
        packages.append({
            ...
            "earliest_days_remaining": max(0, PSC_WINDOW_DAYS - (pd.Timestamp.now() - row["earliest"]).days) if pd.notna(row["earliest"]) else 0,
            "latest_days_remaining": max(0, PSC_WINDOW_DAYS - (pd.Timestamp.now() - row["latest"]).days) if pd.notna(row["latest"]) else 0,
